...
dndscv = function(mutations, gene_list = NULL, refdb = "hg19", sm = "192r_3w", kc = "cgc81", cv = "hg19", max_muts_per_gene_per_sample = 3, max_coding_muts_per_sample = 3000, use_indel_sites = T, min_indels = 5, maxcovs = 20, constrain_wnon_wspl = T, outp = 3, numcode = 1, outmats = F, mingenecovs = 500, dc = NULL) {
    ...
    mutations <- mutations[,.(CHROM,POS, REF,ALT,SAMPLE)] # Restricting input matrix to first 5 columns
    mutations[, c("CHROM", "REF", "ALT",'SAMPLE') := lapply(.SD, as.character), .SDcols = c("CHROM", "REF", "ALT",'SAMPLE')] # Factors to character
    mutations[, POS := as.numeric(POS)] # Chromosome position as numeric
    mutations <- mutations[REF != ALT] # Removing mutations with identical reference and mutant base
    ...
    if (nrow(unique(mutations[,.(chr,pos,ref,mut)])) < nrow(mutations)) {
    ...
    # If there are indels we concatenate the tables of subs and indels    
    if (any(nrow(indels))) {
